...
import { ArrowRight, Check, Sparkles, FileText, Search, Zap, Download, Brain, Clock, Lightbulb, Target, PenLine, CircleCheckBig } from "lucide-react"
import Image from 'next/image';

import { Button } from "@/components/ui/button"
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card';
import { cn } from "@/lib/utils"
import LocalHeader from '@/components/layout/Header'; // Using the global Header
import Footer from '@/components/layout/Footer'; // Using the global Footer
...
export default function Home() {
  ...
  return (
    <div ref={ref} className="flex flex-col min-h-screen bg-background text-foreground overflow-x-hidden">
...
function Hero() {
  ...
  return (
    <section ref={ref} className="w-full py-20 md:py-32 bg-gradient-to-br from-background to-secondary/30">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 grid md:grid-cols-2 gap-12 items-center">
        ...
        >
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight text-primary">
            ...
            <span className="text-foreground">
              Generate Blog Posts in Minutes with AI
            </span>
          </h1>
        ...
        <motion.div 
          className="relative aspect-video rounded-xl shadow-2xl overflow-hidden group"
          initial={{ opacity: 0, scale: 0.9 }}
          animate={isInView ? { opacity: 1, scale: 1 } : {}}
          transition={{ duration: 0.8, delay: 0.4 }}
        >
          <Image
            src="https://picsum.photos/seed/ai-innovation/1280/720"
            alt="AI generating innovative blog content ideas"
            layout="fill"
            objectFit="cover"
...
function ProblemSolutionSection() {
  ...
  const problems = [
    { title: "Tired of writer's block?", icon: Zap, description: "The blank page can be intimidating, making it hard to start and stay creative.", delay: 0.2 },
    { title: "Struggling to find time?", icon: Clock, description: "Content creation is time-consuming, especially when juggling multiple responsibilities.", delay: 0.3 },
    { title: "Overwhelmed by SEO basics?", icon: Target, description: "Getting started with SEO can feel complex, delaying your content's reach.", delay: 0.4 },
  ];
  ...
  const solutions = [
    ...
    { title: "Simplifies SEO", icon: Search, description: "Get foundational SEO suggestions without the headache.", delay: 0.4 },
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {problems.map((problem) => (
              <motion.div
                key={problem.title}
                className="flex flex-col text-center shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300 bg-card overflow-hidden rounded-xl"
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {solutions.map((solution) => (
              <motion.div
                key={solution.title}
                className="p-6 bg-card rounded-xl shadow-lg flex flex-col items-center text-center transform hover:-translate-y-1.5 transition-all duration-300"
                initial={{ opacity: 0, y: 20 }}
                animate={isInView ? { opacity: 1, y: 0 } : {}}
                transition={{ duration: 0.5, delay: solution.delay }}
              >
                <div className="p-3 bg-accent/10 rounded-full mb-4 shadow-sm">
...
function HowItWorksSection() {
  ...
  const steps = [
    { title: "1. Spark Ideas", icon: Sparkles, description: "Enter your keywords and get instant blog title & outline suggestions from our AI blog generator.", imageSeed: "step1", imageAlt: "Visual for Spark Ideas", aiHint: "keyword input idea list", delay: 0.2 },
    { title: "2. Draft Instantly", icon: PenLine, description: "Select an idea, and let our AI writer draft your blog post in minutes.", imageSeed: "step2", imageAlt: "Visual for Draft Instantly", aiHint: "text editor AI draft", delay: 0.4 },
    { title: "3. Optimize Smartly", icon: SearchCheck, description: "Get AI-powered meta title and description suggestions for better SEO content.", imageSeed: "step3", imageAlt: "Visual for Optimize Smartly", aiHint: "SEO suggestions form", delay: 0.6 },
    { title: "4. Export & Go", icon: Download, description: "Download your content easily and get ready to publish on your platform.", imageSeed: "step4", imageAlt: "Visual for Export & Go", aiHint: "export options dialog", delay: 0.8 },
  ];

  return (
    <section id="how-it-works" ref={ref} className="w-full py-16 md:py-24">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8 relative">
          ...
          {steps.map((step) => (
            <motion.div
              key={step.title}
              className="flex flex-col text-center shadow-xl hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300 group relative z-10 bg-card overflow-hidden rounded-xl"
...
function CoreFeaturesSection() {
  ...
  const features = [
    { title: "AI Idea Generation", icon: Lightbulb, description: "Never run out of topics. Enter a keyword and BlogCraft AI instantly provides engaging titles and structured outlines.", benefit: "Benefit: End writer's block, continuous topic supply.", delay: 0.15 },
  ...
  return (
    <section ref={ref} className="w-full py-16 md:py-24 bg-secondary/30">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <motion.h2 
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {features.map((feature) => (
            <motion.div
              key={feature.title}
              className="shadow-lg hover:shadow-xl border border-transparent hover:border-primary/30 transform hover:-translate-y-1.5 transition-all duration-300 rounded-xl bg-card"
              initial={{ opacity: 0, y: 20 }}
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: feature.delay }}
            >
              <CardHeader className="flex flex-row items-center gap-4 p-6">
                <div className="p-3 bg-accent/10 rounded-full shadow-sm">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          {audiences.map((audience) => (
            <motion.div
              key={audience.name}
              className="flex items-center gap-2 bg-card p-4 rounded-lg shadow-md border border-border hover:border-primary/50 transform hover:scale-105 hover:shadow-lg transition-all duration-200"
              initial={{ opacity: 0, scale:0.9 }}
              animate={isInView ? { opacity: 1, scale:1 } : {}}
              transition={{ duration: 0.3, delay: audience.delay }}
            >
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        >
          <Button asChild size="lg" className="bg-background text-primary hover:bg-background/90 shadow-xl transform hover:scale-105 transition-transform duration-200">
            <Link href="/signup">
              Try BlogCraft MVP Now <ArrowRight className="ml-2 h-5 w-5" />
            </Link>
          </Button>
