...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const processKeywordsWithIntent = async (keywords: KeywordMetric[], type: string): Promise<KeywordMetric[]> => {
      ...
      const promises = keywords.map(async (metric) => {
        try {
          if(!metric || typeof metric.keyword !== 'string') {
            ...
            return { ...metric, keyword: metric?.keyword || 'unknown_keyword', intent: 'Unknown' };
          }
          ...
          return { ...metric, keyword: metric?.keyword || 'unknown_keyword', intent: 'Error' };
        }
      });
